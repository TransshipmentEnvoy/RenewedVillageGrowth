...
// Use bundled pathfinder instead of BaNaNaS import
// import("pathfinder.road", "_RoadPathFinder_private_RPF", 4);


/*
 * PUBLIC API
 *
 * For the public API, skip down to: class RoadPathFinder
 */
...
class _RoadPathFinder_private_CustomRPF extends RoadPF
{
...
function _RoadPathFinder_private_CustomRPF::InitializePath(sources, goals)
{
    ::RoadPF.InitializePath(sources, goals);
}

function _RoadPathFinder_private_CustomRPF::SetForbiddenTiles(squirrel_tile_array)
{
    this.forbidden_tiles = squirrel_tile_array;
}

function _RoadPathFinder_private_CustomRPF::SetEstimateMultiplier(estimate_multiplier)
{
    this.estimate_multiplier = estimate_multiplier;
}

function _RoadPathFinder_private_CustomRPF::_Cost(self, path, new_tile, new_direction)
{
    local cost = ::RoadPF._Cost(self, path, new_tile, new_direction);
    local M = 10000;
...
function _RoadPathFinder_private_CustomRPF::_Neighbours(self, path, cur_node)
{
    local tiles = ::RoadPF._Neighbours(self, path, cur_node);
...
function _RoadPathFinder_private_CustomRPF::_Estimate(self, cur_tile, cur_direction, goal_tiles)
{
    local min_cost = ::RoadPF._Estimate(self, cur_tile, cur_direction, goal_tiles);
...
class _RoadPathFinder_private_CustomRPF_NoBuild extends _RoadPathFinder_private_CustomRPF
{
    forbidden_tiles = null;
    estimate_multiplier = null;
...
/*function _RoadPathFinder_private_CustomRPF_NoBuild::InitializePath(sources, goals)
{
    ::_RoadPathFinder_private_RPF.InitializePath(sources, goals);
}

function _RoadPathFinder_private_CustomRPF_NoBuild::SetForbiddenTiles(squirrel_tile_array)
{
    this.forbidden_tiles = squirrel_tile_array;
}

function _RoadPathFinder_private_CustomRPF_NoBuild::SetEstimateMultiplier(estimate_multiplier)
{
    this.estimate_multiplier = estimate_multiplier;
}
*/

function _RoadPathFinder_private_CustomRPF_NoBuild::_Cost(self, path, new_tile, new_direction)
{
    return ::RoadPF._Cost(self, path, new_tile, new_direction);
}

function _RoadPathFinder_private_CustomRPF_NoBuild::_Neighbours(self, path, cur_node)
{
...
function _RoadPathFinder_private_CustomRPF_NoBuild::_Estimate(self, cur_tile, cur_direction, goal_tiles)
{
    local min_cost = ::RoadPF._Estimate(self, cur_tile, cur_direction, goal_tiles);
    return (min_cost * self.estimate_multiplier).tointeger();
}
...
class RoadPathFinder
{
    ...
    {
        if(!no_build)
            this._pf = _RoadPathFinder_private_CustomRPF();
        else
            this._pf = _RoadPathFinder_private_CustomRPF_NoBuild();
    }
...
function RoadPathFinder::InitializePath(sources, goals, repair_existing = false, forbidden_tiles = [], estimate_multiplier = 1)
{
    ...
    this.find_path_error_code = RoadPathFinder.PATH_FIND_NO_ERROR;

    // Defalut params
    this.SetStepSize(100);
}

function RoadPathFinder::SetMaxIterations(max_iterations)
{
    this.max_iterations = max_iterations;
}

function RoadPathFinder::SetStepSize(step_size)
{
    this.step_size = step_size;
}

function RoadPathFinder::FindPath(num_iterations = null)
{
    ...
    if(num_iterations_to_run <= 0)
    {
        ...
        Log.Info("SuperLib: Finding path took to long -> abort", Log.LVL_SUB_DECISIONS);
        //Log.Info("SuperLib: Finding path took to long -> abort - used " + this.accumulated_iterations + " of max " + this.max_iterations, Log.LVL_SUB_DECISIONS);
        this.find_path_error_code = RoadPathFinder.PATH_FIND_FAILED_TIME_OUT;
        return null;
    }

    //Log.Info("SuperLib: used " + this.accumulated_iterations + " of max " + this.max_iterations + ". Run " + num_iterations_to_run + " more loops", Log.LVL_SUB_DECISIONS);
    ...
    if(result == false) {
        ...
        this.find_path_error_code = RoadPathFinder.PATH_FIND_FAILED_NO_PATH;
    }

    return result;
}

function RoadPathFinder::GetFindPathError()
{
    return this.find_path_error_code;
}

function RoadPathFinder::GetIterationsUsed()
{
